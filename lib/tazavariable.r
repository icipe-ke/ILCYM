q.c.rate<-function(dato1,dato2)
{
 ################################
 # Encontrando la etiqueta muerte

 vals=(labels(table(c(as.matrix(dato1[,])))))[[1]]
 muerte=vals[length(vals)]

 dF=dim(dato1)
 dM=dim(dato2) #DM: cambie dM=dim(dato1) por dM=dim(dato2)

 #################################################################
 # viendo si hay diferente numero de insectos por cada temperatura

 if(dM[1]!=dF[1]){stop("the number of rows are different")}
 if(dM[2]!=dF[2]){stop("the number of columns are different")}
 ...
 ind=1:length(tmvF)
 cat("\n","These insects are killed on different days and being","\n","the same insect, according to the order of reproduction","\n","table of females and males","\n","\n")
 #print(data.frame(Position=ind[tmvF!=tmvM],Temperature=datF[tmvF!=tmvM,1]))

 print(data.frame(Insect=ind[tmvF!=tmvM],Ovip.Time.F=tmvF[tmvF!=tmvM],Ovip.Time.M=tmvM[tmvF!=tmvM],Temperature=datF[tmvF!=tmvM,1]))
...
# cat("\n","\n","CHI SQUARE TEST FOR A FEMALE RATIO IN THE REPRODUCTION","\n")
# cat("\n","\n","Hypothesis","\n")
# cat("\n","Ho: Pi=Pm")
# cat("\n","Ha: Pi is non linear function g(Ti)")
# cat("\n","alfa=0.05","\n")
# cat("\n","i=1,2,..k    where K is temperatures number and Pm is","\n","a fixed value","\n")

 TasaT=sum(oviF)/sum(oviF+oviM)
# cat("\n","The fixed value is: ",TasaT,"\n","\n")

# print(data.frame(Fem.Rat.Obs=Tasa,Fem.Rat.Expec=TasaT))

# corrx2=seq(0,max(temps)+5,5);corry2=seq(0,1,0.1)
# plot(temps,Tasa,xlab="Temperature (°C)",ylab="Female ratio in the reproduction",cex=1.1,xlim=c(0,max(temps)+5),ylim=c(0,1),axes=F,xaxt = "n",pch=19,col="blue")
# axis(1, corrx2)  ## cambio
# axis(2,corry2,las=2)
# lines(c(0,max(temps)+5),c(TasaT,TasaT),lwd=2,col="red")
  
# cat("\n","\n","CHI SQUARE TEST","\n")
 oviE=(oviF+oviM)*TasaT
# print(chi.test.rate(Tasa,rep(TasaT,length(temps))))  ## Verificar este resultado
 return(list(Tasa=Tasa,temps=temps,TasaP=TasaT,oviF=oviF,oviM=oviM))
}
...
prueba.rate<-function(x,y,model,inis)
{
 plot(x,y)
     modls=model
     ## Aqui se definen las funciones de los modelos
     if(modls==1){f1=as.formula(y ~ b0 + b1*x + b2*x^2);ff <- function(x){b0 + b1*x + b2*x^2}}
     if(modls==2){f1=as.formula(y ~ (exp(a+b*x))/(1+exp(a+b*x)));ff <- function(x){(exp(a+b*x))/(1+exp(a+b*x))}}
     if(modls==3){f1=as.formula(y ~  c*exp((-0.5)*((x-u)^2)/(t^2)));ff <- function(x){c*exp((-0.5)*((x-u)^2)/(t^2))}}
     for (k in names(inis))
     {
    	temp <- inis[[k]]
	...
	assign(k, temp)
     ...
     curve(ff,add=TRUE,col="royalblue")  ## 2
 return(list(inis=inis,f1=f1,ff=ff))
}


model.rate<-function(x,y,inis,f1,ff,corrx=NULL)
{
 if(is.null(corrx)){corrx=c(0,max(x)+max(x)*0.1)} ## nuevo
 corrx2=seq(corrx[1],corrx[2]+5,5) ## cambio

 plot(x,y,xlab="temperature (°C)",ylab="Rate of oviposition",cex=1.1,xlim=corrx,ylim=c(0,1),axes=F,xaxt = "n",pch=19)
 axis(1, corrx2)  ## cambio
 axis(2,las=2)
     ...
     modsel<-data.frame(R2=round(r,3),R2_Adj=round(r_ajus,3),AIC=round(AC,3))
...
#####################
 ...
 chi.test.rate<-function(oviF,oviE)
 {
   ...
   Res=data.frame(x²=x2,P.Value=pval)
   return(Res)
 }
...
#	if(modelm==36) form<-expression((w-xlinea)^(-1))
  ...
  if(modelm==46) form<-expression(cte+0*xlinea) #DM: MODELO CONSTANTE (NUEVO)
...
dead_func <- function (proc,modelm, datm, alg, inim, pesos,weights){
	...
	if(alg=="Marquardtr")
	{
		...
		if(modelm==45){
 		...
 		if(modelm==46) { ##DM: MODELO CONSTANTE (NUEVO)
			f <- function(x,cte)
			{
				expr <- expression(cte+0*x)
				eval(expr)
			}
			j <- function(x,cte)
			{
			...
			g<-y ~ cte
			if(proc=="mortal") frm<- "m(T) = cte" else frm<- "f(T) = cte"
		}
...
###################################################################################################################################################
coef_mort<-function(proc,modelm,estimor,stdmortg,modelo,gg,datm,alg,pesos,weights)
{
	x<-datm[,1]
	y<-datm[,2]
	ind <- as.list(estimor)
...
#			colnames(FRAMESEL)[4]<-nombre # Nuevo DM
	...
	if(alg=="Marquardtr")
	{
		...
		if(modelm==45) {
 		...
 		if(modelm==46) { ##DM: MODELO CONSTANTE (NUEVO)
			expre<-expression(cte+0*x)
			yl<-eval(expre)
			gg<-y ~ cte
		}

		sqe <- sum((y-yl)^2)+0.000000000000000001 #DM: Artificio +0.000000000000000001
...
grafmort <- function(proc, modelm, estimor, g, datm, corrx=NULL, corry=NULL, mini, maxi, limit,tam,labx, laby,titulo,grises=FALSE, scaleX, scaleY){
	...
	if(modelm==46) { ##DM: MODELO CONSTANTE (NUEVO)
		exprex<-expression(cte+0*x)
		ylx<-eval(exprex)
		expre<-expression(cte+0*xl)
		yl<-eval(expre)
	}
...
#DM
################################################################
# Arreglo para datos de tabla de vida completa con tasa variable  
################################################################

###VERSION6

  tamn<-function(vect){length(vect[is.na(vect)])}
  rr<-function(vect,k){c(vect,rep(NA,k))}
  addovi<-function(datos,estadios) {
    tem1<-tem2<-tem3<-tem4<-list(1); tttt<-c(1)
    muerte=as.character(datos[[1]][nrow(datos[[1]]),ncol(datos[[1]])])
    for(i in 1:length(datos)) {
      for(j in 1:length(estadios)) { tem1[[i]]=t(rm.dead(datos[[i]],estadios[j])) 
      }
      tt<-apply(tem1[[i]],1,tamn)
      tt<-(1:length(tt))[tt!=nrow(datos[[i]])]
      tem2[[i]]<-tem1[[i]][tt,]  
      ttt<-apply(tem2[[i]],2,tamn)
      ttt<-(1:length(ttt))[ttt!=nrow(tem2[[i]])]
      tem3[[i]]<-tem2[[i]][,ttt]
    }
    num<-sapply(tem3,ncol)
    dif<-max(num)-num
    pri<-max(num)-dif
    for(i in 1:length(datos)) {
      tem4[[i]]<-t(apply(tem3[[i]],1,rr,dif[i]))
      if(length(tem4[[i]])==0)  tem4[[i]]<-matrix(NA,1,max(num))
    }
    tem5<-as.matrix(cbind(rep(tp[1],nrow(tem4[[1]])),tem4[[1]]))
    for(i in 2:length(datos)) {
      tem5<-rbind(tem5,cbind(rep(tp[i],nrow(tem4[[i]])),tem4[[i]]))
    }
    tem5<-as.matrix(cbind(tem5,rep(NA,nrow(tem5))))
    for(i in 1:nrow(tem5))  {
      tttt<-na.omit((1:ncol(tem5))[tem5[i,]!="NA"])
      ind<-tttt[length(tttt)]+1
      for(j in ind:ncol(tem5)) {
        tem5[i,j]<-muerte
      }
    }
    tem6<-as.data.frame(tem5)
    return(tem6)    
    }
...
#Male<-data.frame(cbind(rbind(rep(tp[1],length(
