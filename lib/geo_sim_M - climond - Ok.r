...
maps.indic<-function(ptm,modelim=modelim,modelm=modelm,estadios=estadios,xi=xi,steps=steps,m=2,RasterFiles=RasterFiles)
{
 ...
 coordinates(Temp1) <- ~x+y 
	...
	r=raster(nrows=800, ncols=ncols, xmn=-187, xmx=187, ymn=-68, ymx=68)
	r[] <- rep(-9999,ncell(r))
	mm <- extent(min(IND1[,1])-1, max(IND1[,1])+1, min(IND1[,2])-1, max(IND1[,2])+1)
	r <- crop(r, mm)
