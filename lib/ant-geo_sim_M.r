...
##########################################
RateI<-function(vec,Table2,K,parametrosc,parametrosm=NULL,funciont,funcionm=NULL,nmax,steps,J=NA)
{
	for (i in names(parametrosc)){temp <- parametrosc[i];storage.mode(temp) <- "double";assign(i, temp)}
	i=0:(steps-1)
...
#  print(modelim[K])
	if((modelim[K]==1 || modelim[K]==2 || modelim[K]==3 ||
				...
				modelim[K]==14) & is.na(J)){x = T1 + 273.15;x2 = T2 + 273.15}else{x = T1;x2 = T2}
