...
VIDA<-function(vidalife,Tablelife,p,cuadro,estadios,N,M,hfeno,Rs,Rs2=NULL){
	...
	for(k in 1:NUM){
		...
		RESM[,k]<-as.numeric(as.character((statistlife(estimado[[k]],estadios)$Mortality)[,1]))
	}
	...
	MEANresm<-apply(RESM,1,mean) ## este vector contiene el porcentaje promedio de mortalidad de cada estado inmaduro
	...
	colnames(p)<-"Observed";t95=abs(qt(0.05/2,3))
	
	P<-c(1)
	for(s in 1:6){P[s]<-t.test(tablita[s,],mu=p[s,],alt="two.side")$p.value} ## p-value de la prueba de hipotesis de los parametros
...
#LifeTable<-function(cuadro, estadios, n, m, hfeno, sexratio){
    ...
    for(z in 1:n){
        ...
        while(dead != 1 && k != numMale){
            ...
            Insect_age = Insect_age + djx
            dev_p <- distriModelAdults(Insect_age,k)
            #dev_p = log(Insect_age)
            #dev_p <- distrimodel(dev_p,k)
