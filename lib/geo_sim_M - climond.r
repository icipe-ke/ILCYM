...
#########################################################
zone.div<-function(dir1,dir2,ilon,ilat,R,dir.out,name.out=name.out,method=method,modelim=modelim,modelm=modelm,estadios=estadios,xi=xi,steps=steps,filtro=NULL,DL=NULL){
	...
	for(j in R:1){
		...
		#system.time(
				...
				for(i in 1:nfil){
					...
					RFinal[i,]=GenActIndex(i,TempsAll=TempsAll,coords=coords,x1=x1,y1=y1,method=method,modelim=modelim,modelm=modelm,estadios=estadios,xi=xi,steps=steps,filtro,DL)$indices
				}
...
GenActIndex<-function(posic,TempsAll=TempsAll,coords=coords,x1=x1,y1=y1,method=method,modelim=modelim,modelm=modelm,estadios=estadios,xi=xi,steps=steps,filtro=NULL,DL=NULL)
{
	...
	{
	   if(!is.null(filtro)){tmm=mean(Table);if(tmm[1] > filtro[1] && tmm[2] < filtro[2]){filtroin=TRUE}else{filtroin=FALSE}}
	   if(filtroin)
	   {
...
#############################################################
#############################################################
maps.indic<-function(ptm,modelim=modelim,modelm=modelm,estadios=estadios,xi=xi,steps=steps,m=2,RasterFiles=RasterFiles)
{
 ...
 indic2=sapply(Tables,GenActIndex.uni.mens,modelim=modelim,modelm=modelm,estadios=estadios,xi=xi,steps=steps)
 IND1=data.frame(ptm1[,-3],loc=loc,t(data.frame(indic2)));rownames(IND1)=1:nrow(IND1);colnames(IND1)[3]="alt"

 IND1=IND1[IND1[,1]>-179.9 & IND1[,1]<179.9,]
 #IND1=IND1[IND1[,1]>-179.90,]
 #IND1=IND1[IND1[,2]<179.91,]


 #write.table(IND1,"Indices_Temp_Observ.txt")

 Temp1=data.frame(IND1) ## observados
 coordinates(Temp1) <- ~x+y 

	library(raster)
	resol=0.17
	ncols=374/resol # 2200
	r=raster(nrows=1000, ncols=ncols, xmn=-187, xmx=187, ymn=-85, ymx=85)
	r[] <- rep(-9999,ncell(r))
	mm <- extent(min(IND1[,1])-0.1, max(IND1[,1])+0.1, min(IND1[,2])-1, max(IND1[,2])+1)
	r <- crop(r, mm)

	res(r) <- c(resol, resol)

	detach(package:raster) # dejamos de usar la libreria para que no halla conflicto
	r<-as(r,'SpatialPixelsDataFrame')
	ind1 <- overlay(r,Temp1)## vector - funciona pues A:SpatialPointsDataFrame B:SpatialPixelsDataFrame

	#AI
	r1=r
	tmp1=Temp1@data[,3];tmp1[!is.finite(tmp1)]=-9999
	r1@data[ind1,]=tmp1
	r1@data[r1@data[,1] == -9999,1]=NA
	gridded(r1)<- TRUE

	writeAsciiGrid(r1["layer"], na.value = -9999,RasterFiles[1])
	#AI = readAsciiGrid("AI_climex.asc")
	#spplot(AI["AI_climex.asc"],xlim=c(-76,-62),ylim=c(-57,-45),scales = list(draw = TRUE))


	#GI
	r1=r
	tmp1=Temp1@data[,4];tmp1[!is.finite(tmp1)]=-9999
	r1@data[ind1,]=tmp1
	r1@data[r1@data[,1] == -9999,1]=NA
	gridded(r1)<- TRUE

	writeAsciiGrid(r1["layer"], na.value = -9999,RasterFiles[2])
	#GI = readAsciiGrid("GI_climex.asc")
	#spplot(GI["GI_climex.asc"],xlim=c(-76,-62),ylim=c(-57,-45),scales = list(draw = TRUE))


	#ERI
	r1=r
	tmp1=Temp1@data[,5];tmp1[!is.finite(tmp1)]=-9999
	r1@data[ind1,]=tmp1
	r1@data[r1@data[,1] == -9999,1]=NA
	gridded(r1)<- TRUE

	writeAsciiGrid(r1["layer"], na.value = -9999,RasterFiles[3])
	#ERI = readAsciiGrid("ERI_climex.asc")
	#spplot(ERI["ERI_climex.asc"],xlim=c(-76,-62),ylim=c(-57,-45),scales = list(draw = TRUE))

}
###################################33#################
###################################33#################
###################################33#################
GenActIndex.uni.mens<-function(Table,modelim=modelim,modelm=modelm,estadios=estadios,xi=xi,steps=steps,filtro=NULL,DL=NULL)
{
	...
	if(length(Table[is.na(Table)])==0)
	{
	   if(!is.null(filtro)){tmm=mean(Table);if(tmm[1] > filtro[1] && tmm[2] < filtro[2]){filtroin=TRUE}else{filtroin=FALSE}}else{filtroin=TRUE}
	   if(filtroin)
	   {
		inmaduros <-  estadios[-(length(estadios)-1):-(length(estadios))]
		maduros   <-  estadios[(length(estadios)-1):(length(estadios))]
