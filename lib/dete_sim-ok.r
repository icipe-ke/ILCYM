...
#############################################################################################################
##########################################################################################################
simultemp<-function(N,sexratio,isFixed,temp,resps,xi,steps, poli, ..){
	n1=length(temp)
...
life.tableD<-function(cuadro,estad,n,m,hfeno,Rs,Amp=1)
{
	...
	tab2 <- matrix(0,nrow = m,ncol = 3) ## Siempre son 3 columnas asumiendo que solo se tiene Hembra, Macho y Oviposición
...
###############################################################################
###############################################################################
detrm2<-function(dij,dacc,pij,plij,riter,rest,rate,ovi,tamaño,generacion,dia,Day)
{
N<-tamaño
inmad <-  estadios[-(length(estadios)-1):-(length(estadios))]
maduros   <-  estadios[(length(estadios)-1):(length(estadios))]
